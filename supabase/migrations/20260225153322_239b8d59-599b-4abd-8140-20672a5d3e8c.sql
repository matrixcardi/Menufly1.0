...
-- Fix addon price manipulation by adding bounds validation in submit_order
-- This prevents attackers from sending negative addonsTotal to reduce order costs
...
BEGIN
  ...
  LOOP
    ...
    -- SECURITY FIX: Validate addon total bounds
    -- Addons total must be non-negative and capped at a reasonable maximum per item
    ...
    -- Cap addon total at 150% of product price or R$100, whichever is higher
    -- This prevents absurd values while allowing legitimate addon combinations
