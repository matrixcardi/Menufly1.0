...
-- Drop the 3-overload variants and recreate with p_delivery_fee parameter

-- Overload 1: with promo + auto_promo
DROP FUNCTION IF EXISTS public.submit_order(uuid, text, text, text, text, text, jsonb, text, text, uuid, uuid[]);
...
BEGIN
  ...
  END IF;
  IF LENGTH(v_clean_notes) = 0 THEN v_clean_notes := NULL; END IF;
  ...
  LOOP
    ...
    FROM products
    WHERE (id = (v_item.value->>'productId')::UUID OR id = (v_item.value->>'id')::UUID)
    AND restaurant_id = p_restaurant_id AND is_active = true;
    
    IF NOT FOUND THEN
      RETURN json_build_object('success', false, 'error', 'Produto inválido ou indisponível: ' || COALESCE(v_item.value->>'name', 'Unknown'));
    END IF;
    
    IF (v_item.value->>'quantity')::INTEGER < 1 OR (v_item.value->>'quantity')::INTEGER > 99 THEN
      RETURN json_build_object('success', false, 'error', 'Quantidade inválida para: ' || v_product.name);
    END IF;
    ...
    IF v_addons_total < 0 THEN
      RETURN json_build_object('success', false, 'error', 'Valor de adicionais inválido para: ' || v_product.name);
    END IF;
    IF v_addons_total > GREATEST(v_item_price * 1.5, 100) THEN
      RETURN json_build_object('success', false, 'error', 'Valor de adicionais excede o limite para: ' || v_product.name);
    END IF;
    IF v_addons_total > 0 THEN
    ...
    v_validated_items := v_validated_items || jsonb_build_object(
      'id', v_product.id, 'productId', v_product.id,
      'name', COALESCE(v_item.value->>'name', v_product.name),
      'price', v_item_price, 'quantity', (v_item.value->>'quantity')::INTEGER,
      'addons', COALESCE(v_item.value->'addons', '[]'::JSONB),
      'addonNames', COALESCE(v_item.value->'addonNames', '{}'::JSONB),
      'addonsTotal', v_addons_total, 'notes', v_item.value->>'notes',
  ...
  IF p_promo_id IS NOT NULL THEN
    SELECT * INTO v_promo FROM promos WHERE id = p_promo_id AND restaurant_id = p_restaurant_id AND is_active = true;
    IF FOUND THEN
      IF v_promo.promo_type IN ('fixed_kit', 'choice_kit') AND v_promo.price IS NOT NULL THEN
        SELECT COALESCE(SUM(p.price), 0) INTO v_promo_items_total FROM promo_items pi JOIN products p ON p.id = pi.product_id WHERE pi.promo_id = p_promo_id;
        IF v_promo_items_total > v_promo.price THEN v_discount := v_discount + (v_promo_items_total - v_promo.price); END IF;
      ELSIF v_promo.promo_type = 'auto_discount' AND v_promo.discount_value IS NOT NULL THEN
        ...
        ELSE v_discount := v_discount + LEAST(v_promo.discount_value, v_subtotal); END IF;
      END IF;
    END IF;
  END IF;

  IF p_auto_promo_ids IS NOT NULL AND array_length(p_auto_promo_ids, 1) > 0 THEN
    FOR v_auto_promo IN SELECT * FROM auto_promos WHERE id = ANY(p_auto_promo_ids) AND restaurant_id = p_restaurant_id AND is_active = true
    LOOP
      DECLARE
      ...
      BEGIN
        CASE v_auto_promo.trigger_type
          WHEN 'min_items' THEN
            SELECT COALESCE(SUM((elem->>'quantity')::INTEGER), 0) INTO v_total_items FROM jsonb_array_elements(v_validated_items) elem;
            v_triggered := v_total_items >= v_auto_promo.trigger_value;
          WHEN 'min_value' THEN v_triggered := v_subtotal >= v_auto_promo.trigger_value;
          WHEN 'specific_product' THEN
            v_triggered := EXISTS (SELECT 1 FROM jsonb_array_elements(v_validated_items) elem WHERE (elem->>'productId')::UUID = v_auto_promo.trigger_product_id);
          WHEN 'specific_category' THEN
            SELECT COALESCE(SUM((elem->>'quantity')::INTEGER), 0) INTO v_cat_items FROM jsonb_array_elements(v_validated_items) elem JOIN product_categories pc ON pc.product_id = (elem->>'productId')::UUID WHERE pc.category_id = v_auto_promo.trigger_category_id;
            v_triggered := v_cat_items >= v_auto_promo.trigger_value;
          ELSE v_triggered := false;
        END CASE;
        IF v_triggered THEN
          CASE v_auto_promo.benefit_type
            WHEN 'free_shipping' THEN v_auto_free_shipping := true;
            WHEN 'percentage_discount' THEN v_auto_discount := v_auto_discount + (v_subtotal * (COALESCE(v_auto_promo.benefit_value, 0) / 100));
            WHEN 'fixed_discount' THEN v_auto_discount := v_auto_discount + LEAST(COALESCE(v_auto_promo.benefit_value, 0), v_subtotal);
            WHEN 'free_product' THEN NULL;
            ELSE NULL;
          END CASE;
        END IF;
      END;
  ...
  IF p_delivery_type = 'delivery' THEN
    ...
    ELSIF p_delivery_fee IS NOT NULL THEN
      v_delivery_fee := GREATEST(p_delivery_fee, 0);
    ELSE
      v_delivery_fee := 5.00;
    END IF;
  END IF;
  ...
  INSERT INTO orders (
    ...
    v_validated_items, v_clean_notes, v_daily_number
  ...
  RETURN json_build_object(
    ...
    'daily_number', v_daily_number, 'subtotal', v_subtotal, 'discount', v_discount,
...
-- Drop old overloads that don't have p_delivery_fee
DROP FUNCTION IF EXISTS public.submit_order(uuid, text, text, text, text, text, jsonb, text, text);
DROP FUNCTION IF EXISTS public.submit_order(uuid, text, text, text, text, text, jsonb, text, text, uuid);
