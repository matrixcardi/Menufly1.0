-- Fix submit_order to use restaurant's default_delivery_fee instead of hardcoded 5.00
...
DECLARE
  ...
  v_restaurant_default_delivery_fee DECIMAL := 0;
BEGIN
  ...
  -- Fetch restaurant's default delivery fee
  SELECT COALESCE(default_delivery_fee, 0) INTO v_restaurant_default_delivery_fee
  FROM restaurants
  ...
  IF p_promo_id IS NOT NULL THEN
    ...
    IF FOUND THEN
      IF v_promo.promo_type IN ('fixed_kit', 'choice_kit') AND v_promo.price IS NOT NULL THEN
        SELECT COALESCE(SUM(p.price), 0) INTO v_promo_items_total FROM promo_items pi JOIN products p ON p.id = pi.product_id WHERE pi.promo_id = v_promo_id;
  ...
  IF p_delivery_type = 'delivery' THEN
    ...
    ELSE
      v_delivery_fee := v_restaurant_default_delivery_fee;
    END IF;
  END IF;
  ...
  INSERT INTO orders (
    ...
    coupon_code, items, notes, daily_number, scheduled_at, scheduling_type
  ) VALUES (
    ...
    v_validated_items, v_clean_notes, v_daily_number, p_scheduled_at, p_scheduling_type
