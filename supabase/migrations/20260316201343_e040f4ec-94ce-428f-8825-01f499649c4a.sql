...
CREATE OR REPLACE FUNCTION public.submit_order(
  ...
  p_promo_id uuid DEFAULT NULL::uuid
)
RETURNS json
LANGUAGE plpgsql
SECURITY DEFINER
  ...
  IF p_promo_id IS NOT NULL THEN
    ...
    IF FOUND THEN
      IF v_promo.promo_type IN ('fixed_kit', 'choice_kit') AND v_promo.price IS NOT NULL THEN
        -- Calculate sum of promo item DB prices (for items in the order that belong to this promo)
        SELECT COALESCE(SUM(p.price), 0) INTO v_promo_items_total
        FROM promo_items pi
        JOIN products p ON p.id = pi.product_id
        WHERE pi.promo_id = p_promo_id;
        
        -- The discount is the difference between regular prices and promo price
      ...
      ELSIF v_promo.promo_type = 'auto_discount' AND v_promo.discount_value IS NOT NULL THEN
        -- Auto discount: apply to matching items in the order
