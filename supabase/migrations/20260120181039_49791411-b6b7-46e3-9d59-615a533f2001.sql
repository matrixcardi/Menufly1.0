-- Fix critical price manipulation vulnerability in submit_order
-- The function now validates product prices against the database instead of trusting client-provided prices
...
BEGIN
  ...
  -- Calculate subtotal from items - VALIDATE PRICES AGAINST DATABASE
  FOR v_item IN SELECT * FROM jsonb_array_elements(p_items)
  LOOP
    -- Look up actual product price from database using productId OR id field
    ...
    -- If product not found or inactive, reject order
    IF NOT FOUND THEN
    ...
    -- Use DATABASE PRICE, not client-provided price (SECURITY FIX)
    v_item_price := v_product.price;
    v_subtotal := v_subtotal + (v_item_price * (v_item.value->>'quantity')::INTEGER);
    
    -- For addons, we trust the client total for now since there's no addons table
    -- TODO: Create addons table and validate addon prices server-side
    IF v_item.value->>'addonsTotal' IS NOT NULL AND (v_item.value->>'addonsTotal')::DECIMAL > 0 THEN
      v_subtotal := v_subtotal + ((v_item.value->>'addonsTotal')::DECIMAL * (v_item.value->>'quantity')::INTEGER);
    END IF;
    
    -- Build validated item with correct price from DB
    v_validated_items := v_validated_items || jsonb_build_object(
      'id', v_product.id,
      'productId', v_product.id,
      'name', COALESCE(v_item.value->>'name', v_product.name),
      'price', v_item_price, -- Use validated price
      'quantity', (v_item.value->>'quantity')::INTEGER,
      'addons', COALESCE(v_item.value->'addons', '[]'::JSONB),
      'addonsTotal', COALESCE((v_item.value->>'addonsTotal')::DECIMAL, 0),
  ...
  -- Insert order with VALIDATED items (containing correct prices from DB)
  INSERT INTO orders (
    ...
    v_validated_items, -- Use validated items with correct prices
    v_clean_notes
  )
