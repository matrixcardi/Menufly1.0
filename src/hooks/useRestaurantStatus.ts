    ...
    try {
      ...
      if (restaurantId) {
        ...
        const { data } = await supabase
          .from("restaurants")
          .select("id, user_id, name, address, instagram_url, opening_time, closing_time, min_order, is_open, delivery_available, pickup_available, created_at, updated_at, logo_url, banner_url, meta_pixel_id, gtm_container_id, ga_measurement_id, delivery_method, default_delivery_time_min, restaurant_lat, restaurant_lng, address_cep, address_street, address_number, address_complement, address_neighborhood, address_city, address_state, menu_theme, slug, description, whatsapp_instance_name, whatsapp_connected, manual_override_until, operation_mode, notification_sound, whatsapp_phone, bot_enabled, bot_greeting_message, bot_order_updates, bot_feedback_enabled, google_review_link, pix_enabled, pix_gateway, card_enabled, card_gateway, stripe_publishable_key, cash_enabled, card_on_delivery_enabled, pix_key, mp_public_key, card_online_enabled, delivery_mode, default_delivery_fee, trial_ends_at, trial_email_d7_sent, trial_email_d3_sent, trial_email_d1_sent, trial_email_d0_sent, subscription_active, bot_auto_reply_enabled, free_shipping_threshold, google_maps_url, pix_on_delivery_enabled, pix_gateway_connected")
          .eq("id", restaurantId)
          .maybeSingle();
        restaurantData = data;
      } else {
